...
class Kun(Figurka):
    ...
    def over_tah(self, sachovnice, puvodni_pozice, nova_pozice):
        ...
        mozne_pozice = [
            (puvodni_radek - 1, puvodni_sloupec - 2),
            (puvodni_radek - 1, puvodni_sloupec + 2),
            (puvodni_radek + 1, puvodni_sloupec - 2),
            (puvodni_radek + 1, puvodni_sloupec + 2),
            (puvodni_radek - 2, puvodni_sloupec - 1),
            (puvodni_radek - 2, puvodni_sloupec + 1),
            (puvodni_radek + 2, puvodni_sloupec - 1),
            (puvodni_radek + 2, puvodni_sloupec + 1)]
        if nova_pozice not in mozne_pozice:
            raise ValueError(f'Zde nemůžeš zahrát koněm')
class Kral(Figurka):
    def __init__(self, strana):
    ...
    def over_tah(self, sachovnice, puvodni_pozice, nova_pozice):
        ...
        if abs(novy_radek - puvodni_radek) > 1 or abs(novy_sloupec - puvodni_sloupec) > 1:
            raise ValueError(f'Zde nemůžeš zahrát kralem')
...
class Strelec(Figurka):
    ...
    def over_tah(self, sachovnice, puvodni_pozice, nova_pozice):
        ...
        if abs(novy_radek - puvodni_radek) == abs(novy_sloupec - puvodni_sloupec):
            preskocene_pole = range(abs(novy_radek - puvodni_radek))
            if novy_radek - puvodni_radek > 0:
                radek = True
            else:
                radek = False
            if novy_sloupec - puvodni_sloupec > 0:
                sloupec = True
            else:
                sloupec = False

            for i in preskocene_pole:
                if i != 0:
                    if radek and sloupec:
                        testovana_pozice = novy_radek - i, novy_sloupec - i
                    elif not radek and not sloupec:
                        testovana_pozice = novy_radek + i, novy_sloupec + i
                    elif radek and not sloupec:
                        testovana_pozice = novy_radek - i, novy_sloupec + i
                    elif not radek and sloupec:
                        testovana_pozice = novy_radek + i, novy_sloupec - i
                    blokujici = sachovnice.figurka_na(testovana_pozice)
                    if blokujici != None:
                        raise ValueError(f'V cestě je {blokujici}')
        else:
            raise ValueError(f'Nepovolený tan střelcem')
class Sachovnice:
    def __init__(self):
